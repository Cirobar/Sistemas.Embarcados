...
int main(int argc, char *argv[])
{
	...
	ultr1();
		if(dist0 > 3 && dist0 < 5){
			...
			//read(fd[0],&stexec,1); 	
			stexec = 1;
			system("curl -s -X POST https://api.telegram.org/bot1067349259:AAHQQ2jD7224MYYtyJr8s0aeJONG3aTHqHg/sendMessage -d chat_id=705655937 -d text='A stream foi aberta pelo tof'");
			}
		}
		if(dist1 > 3 && dist1 < 5){
			while(stexec==0){ 
			...
			//read(fd[0],&stexec,1);	
			stexec = 1;
			system("curl -s -X POST https://api.telegram.org/bot1067349259:AAHQQ2jD7224MYYtyJr8s0aeJONG3aTHqHg/sendMessage -d chat_id=705655937 -d text='A stream foi aberta pelo ultr1'");
			}
